    ...
    public class ProcessHealEffectSystem : IExecuteSystem
    {
        ...
        public void Execute()
        {
            foreach (GameEntity effect in _effects)
            {
                ...
                if (target.hasCurrentHealth && target.hasMaxHealth)
                {
                    float newValue = Mathf.Min(target.CurrentHealth + effect.EffectValue, target.MaxHealth);
                    target.ReplaceCurrentHealth(newValue);
                }
            }
        }
    }
